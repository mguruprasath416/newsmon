...
### Pipeline D: Multi-Channel Webhook Notification Engine (Microsoft Teams & Discord)

Monitors incoming company-based cyber news and dispatches adaptive cards to designated Microsoft Teams regional channels (`#indian-based` and `#gcc-middle-east`) and Discord webhooks.

```mermaid
flowchart TD
    News[News Monitoring Ingestion] --> CyberFilter{Company-Based Cyber News?}
    CyberFilter -->|No| Skip[Skip Dispatch]
    CyberFilter -->|Yes| RegionBranch{Regional Classification}

    RegionBranch -->|India| IndiaBranch[India Branch]
    IndiaBranch --> IndComp[Indian-Based Companies]
    IndiaBranch --> ForIndComp[Foreign Companies in India]

    RegionBranch -->|Middle-East & GCC| MEBranch[Middle-East & GCC Branch]
    MEBranch --> MEComp[Middle-East Companies]
    MEBranch --> GCCComp[GCC Companies]
    MEBranch --> ForMEComp[Foreign Companies in Middle East]

    IndComp --> IndTeamsCard[Message Card -> #indian-based Channel]
    ForIndComp --> IndTeamsCard

    MEComp --> METeamsCard[Message Card -> #gcc-middle-east Channel]
    GCCComp --> METeamsCard
    ForMEComp --> METeamsCard

    News --> DiscordWebhook[Discord Embed Dispatch]
```

#### Microsoft Teams Message Card Field Specification
Every card sent to Microsoft Teams strictly contains 4 structured facts:
1. **`Source:`** `[source_name](article_url)` (Source Name + Link)
2. **`Date of Publish:`** Published date string (e.g. `13 Aug 2026`)
3. **`Threat Actor:`** Canonical threat actor name (`Name` else `Unknown`)
4. **`Company:`** Victim/targeted organization (`Name` else `Unknown`)
